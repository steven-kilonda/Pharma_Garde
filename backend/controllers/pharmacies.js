...
export async function getAllPharmacies(req, res, next) {
  try {
    ...
    console.error(
      "Erreur récupération pharmacies :",
      error
    );
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Le code, le nom et l'arrondissement sont obligatoires"
      });
    }
    ...
    const listeTelephones =
      Array.isArray(telephones)
        ? telephones
        : [];
    ...
    if (!arrondissementResult.rows.length) {
      return res.status(400).json({
        error:
          "L'arrondissement sélectionné n'existe pas"
      });
    }
    ...
    for (
      const [jour, horaire]
      of Object.entries(listeHoraires)
    ) {
